...
class NumMatrix(_matrix: Array[Array[Int]]) {
  ...
  def update(row: Int, col: Int, x: Int) : Unit = {
    val arr = rowSum.get(row).get
    //println(arr.mkString(","))

    arr(col) = arr(col) + (x-_matrix(row)(col))
    for (j <- col+1 to _matrix(0).length-1) {
      arr(j) = arr(j-1) + _matrix(row)(j)
    }
