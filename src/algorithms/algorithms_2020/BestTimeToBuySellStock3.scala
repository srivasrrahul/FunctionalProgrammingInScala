object Solution {
  def maxProfit(prices: Array[Int]): Int = {
    if (prices.isEmpty) {
      ...
      val matrix = Array.ofDim[Int](2, prices.length)
      ...
      for (j <- 1 to matrix(0).length - 1) {
        ...
        matrix(0)(j) = maxProfitToday
        if (j == 1 ) {
          maxProfitForSingleSell(1) = matrix(0)(j)
        }else {
          maxProfitForSingleSell(j) = scala.math.max(maxProfitForSingleSell(j-1),matrix(0)(j))
        }
      }
      ...
      Array(0, matrix(0).max, matrix(1).max).max
    }
